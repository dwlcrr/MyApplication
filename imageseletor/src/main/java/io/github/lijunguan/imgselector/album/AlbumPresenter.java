...
public class AlbumPresenter implements AlbumContract.Presenter {
        ...
        switch (requestCode) {
            case ImageSelector.REQUEST_OPEN_CAMERA:
                if (resultCode == Activity.RESULT_OK) {
                    if (ImageSelector.getInstance().getConfig().isCrop()) {
            ...
            case ImageSelector.REQUEST_CROP_IMAGE:
                ...
                break;
            case ImageSelector.REQUEST_SINGLE_IMAGE:
                if (resultCode == Activity.RESULT_OK) {
                    String path = data.getStringExtra(ShowOnePhotoActivity.SINGLE_RESULT);
                    mAlbumRepository.addSelect(path);
                    mAlbumView.selectComplete(mAlbumRepository.getSelectedResult(), false);
                }
                break;
    ...
    @Override
    public void openCamera(int maxCount) {
        if (mAlbumRepository.getSelectedResult().size() >= maxCount) {
            mAlbumView.showOutOfRange(-1);
            return;
        }
        openCamera();
    }
