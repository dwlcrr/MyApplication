...
public class AlbumFragment extends BaseFragment
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //根据选择模式，打开不同的UI
        int status = ImageSelector.getInstance().getConfig().getSelectModel();
        if (status == ImageSelector.SINGLE_MODE) {
            mContext.setSubmitGone();
        }
        //如果是拍照模式,直接打开相机
        if (status == ImageSelector.CAMERA_MODE) {
    ...
    ImageItemListener mItemListener = new ImageItemListener() {
        ...
        @Override
        public void onCameraItemClick(int maxCount) {
            mPresenter.openCamera(maxCount);
        }
...
//        }
    ...
    public void showSystemCamera() {
        ...
        if (cameraIntent.resolveActivity(mContext.getPackageManager()) != null) {
            ...
            if (mTmpFile != null && mTmpFile.exists()) {
                cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, FileUtils.getFileUri(getContext(), mTmpFile));
    ...
    public void showImageCropUi(@NonNull String imagePath) {
        int status = ImageSelector.getInstance().getConfig().getSelectModel();
        if (status == ImageSelector.SINGLE_MODE && !ImageSelector.getInstance().getConfig().isCrop()) {
            Intent intent = new Intent(getActivity(), ShowOnePhotoActivity.class);
            intent.putExtra("imagePath", imagePath);
            startActivityForResult(intent, ImageSelector.REQUEST_SINGLE_IMAGE);
        } else if ((status == ImageSelector.SINGLE_MODE && ImageSelector.getInstance().getConfig().isCrop()) ||
                ((status == ImageSelector.CAMERA_MODE && ImageSelector.getInstance().getConfig().isCrop()))) {
    ...
    public interface ImageItemListener {
        ...
        void onCameraItemClick(int maxCount);
