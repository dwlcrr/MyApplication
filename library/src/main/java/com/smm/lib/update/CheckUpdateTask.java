...
public class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    private static final String url = FinalConstants.UPDATE_URL;
    ...
    private void parseJson(String result) {
        try {
            ...
            String updateMessage = obj.getString(FinalConstants.APK_UPDATE_CONTENT);
            String apkUrl = obj.getString(FinalConstants.APK_DOWNLOAD_URL);
            int apkCode = obj.getInt(FinalConstants.APK_VERSION_CODE);

            int versionCode = AppUtils.getVersionCode(mContext);

            if (apkCode > versionCode) {
                if (mType == FinalConstants.TYPE_NOTIFICATION) {
                    showNotification(mContext, updateMessage, apkUrl, apkCode);
                } else if (mType == FinalConstants.TYPE_DIALOG) {
            ...
            Log.e(FinalConstants.TAG, "parse json error");
        }
    }
    ...
    private void showNotification(Context context, String content, String apkUrl, int apkCode) {
        ...
        myIntent.putExtra(FinalConstants.APK_DOWNLOAD_URL, apkUrl);
