...
public class SmmTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link SmmTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link SmmTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    private int tabIndicatorWidth = 0; ///  选择线的长度  0 的时候与字同宽    设置后 用这个宽度
    ...
    private boolean drawIndicatorbackground = true;

    // Pool we use as a simple RecyclerBin
    private final Pools.Pool<TabView> mTabViewPool = new Pools.SimplePool<>(12);

    public SmmTabLayout(Context context) {
        this(context, null);
    }

    public SmmTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SmmTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setTabIndicatorWidth(int indicatorWidth) {
        this.tabIndicatorWidth = DisplayUtils.dip2px(getContext(),indicatorWidth);
    }
    ...
    /**
     * Returns the current mode used by this {@link SmmTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link SmmTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private final Paint mBackIndicatorColor;
        ...
        SlidingTabStrip(Context context) {
            ...
            mBackIndicatorColor = new Paint();
            mBackIndicatorColor.setColor(0xfff2f2f2);
        }
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (selectedTitle instanceof TabView) {
                    ...
                    Paint p = new Paint();
                    p.setTextSize(tabView.mTextView.getTextSize());
                    mIndicatorWidth = (int) (p.measureText(tabView.mTab.mText.toString()) + 0.5);
                } else {
                    mIndicatorWidth = Integer.MAX_VALUE;
                }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                if (drawIndicatorbackground) {
                    canvas.drawRect(0, getHeight() - mSelectedIndicatorHeight,
                            getWidth(), getHeight(), mBackIndicatorColor);
                }
                if (tabIndicatorWidth != 0) {
                    int mIndicator = (mIndicatorRight - mIndicatorLeft - tabIndicatorWidth) / 2;
                    canvas.drawRect(mIndicatorLeft + mIndicator, getHeight() - mSelectedIndicatorHeight,
                            mIndicatorRight - mIndicator, getHeight(), mSelectedIndicatorPaint);
                } else if (mIndicatorRight - mIndicatorLeft > mIndicatorWidth) {
                    int mIndicator = (mIndicatorRight - mIndicatorLeft - mIndicatorWidth) / 2;
                    canvas.drawRect(mIndicatorLeft + mIndicator, getHeight() - mSelectedIndicatorHeight,
                            mIndicatorRight - mIndicator, getHeight(), mSelectedIndicatorPaint);
                } else {
        ...
        public void setBackIndicatorColor(int color) {
            if (mBackIndicatorColor != null) {
                mBackIndicatorColor.setColor(color);
            }
        }
    }
    ...
    public void setBackIndicatorColor(int color) {
        if (mTabStrip != null) {
            mTabStrip.setBackIndicatorColor(color);
        }
    }
    ...
    public void setDrawIndicatorbackground(boolean isDraw) {
        this.drawIndicatorbackground = isDraw;
    }

    /**
     * A {@link ViewPager.OnPageChangeListener} class which contains the
     * necessary calls back to the provided {@link SmmTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<SmmTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(SmmTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final SmmTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
